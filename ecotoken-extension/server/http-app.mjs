...
function makeSnowflakeJWT() {
  ...
  const key = SF_PRIVATE_KEY_PASSPHRASE
    ? {
        key: fs.readFileSync(new URL(SF_PRIVATE_KEY_PATH, import.meta.url)),
        passphrase: SF_PRIVATE_KEY_PASSPHRASE,
      }
...
function corsHeaders(origin) {
  const allowed = new Set([EXTENSION_ID ? `chrome-extension://${EXTENSION_ID}` : null, ALLOW_ORIGIN || null, "http://localhost:3001"].filter(Boolean));
    ...
    if (req.method === "POST" && req.url === "/api/optimize") {
      try {
        ...
        if (!tag) {
          ...
          res.end(JSON.stringify({ error: "MISSING_TAG" }));
          return;
        }

        console.log(`[EcoToken API] /optimize`, { tag, rawTextLen: raw_text?.length ?? 0 });
        try {
          await insertPromptSample(tag, raw_text);
        } catch {}
        ...
        res.end(JSON.stringify(out || { error: "NO_RESULT" }));
        return;
      } catch (e) {
        console.error(`[EcoToken API] handler error`, e);
        res.writeHead(500, { ...cors, "Content-Type": "application/json" });
        res.end(JSON.stringify({ error: "BACKEND_EXCEPTION", detail: String(e) }));
        return;
      }
    }
